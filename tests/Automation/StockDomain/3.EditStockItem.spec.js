...
//    await addStockItems.clickOnFreeTextButton()
   await addStockItems.enterNonSnomedCode(jsonData.EditStockItem[0].stock_que_local_code)   
   await addStockItems.enterItemBarcode(jsonData.EditStockItem[0].stock_barcode)
...
//    await addStockItems.enterForm()
 ...
 //  await addStockItems.enterFrequency()
  // await addStockItems.enterRoute(jsonData.EditStockItem[0].stdo_route)
   await addStockItems.enterDuration(jsonData.EditStockItem[0].stdo_duration)
//    await addStockItems.enterDilutant('Yes')
//    await addStockItems.enterDoseMultiplier()
//    await addStockItems.enterStandardAmountToDispense(jsonData.EditStockItem[0].stdo_num_to_dispensed)
...
//    await addStockItems.toggleSuitableForHomeDelivery()
   ...
   await addStockItems.enterInStockQuantity(jsonData.EditStockItem[0].stbat_quantity)
  // await addStockItems.enterSerialNumber(jsonData.EditStockItem[0].stbat_serial_number)
   await page.waitForTimeout(1500)
  //  await addStockItems.enterSterilizationExpiryDate('01/01/2030')
  //  await page.waitForTimeout(1500)
   await addStockItems.enterManufacturedDate(jsonData.EditStockItem[0].stbat_manufacture_date)
   await page.waitForTimeout(1500)
   await addStockItems.enterReceivedDate(jsonData.EditStockItem[0].stbat_batch_received_date)
   await page.waitForTimeout(1500)
   await addStockItems.enterExpiryDate(jsonData.EditStockItem[0].stbat_expiry_date)
  ...
  //  await addStockItems.FillPositions4()
   ...
   //check DB
    var sqlQuery =  "select si.stock_name,si.stock_category,si.stock_barcode,si.stock_item_barcode,si.stock_description,si.stock_desc_other_lang, sb.stbat_serial_number,sb.stbat_quantity,sb.stbat_manufacture_date,sb.stbat_expiry_date,sb.stbat_batch_number,sb.stbat_supplier,sb.stbat_batch_received_date,sb.stbat_purchase_price,sb.stbat_unit_cost,sb.stbat_retail_price from c4_stock_items si JOIN stock_batches sb ON si.stock_id = sb.stbat_stock_id ORDER BY stock_id DESC limit 1";
        ...
        var match = await compareJsons(sqlFilePath, null, jsonData.EditStockItem[index]);
        if (match) {
